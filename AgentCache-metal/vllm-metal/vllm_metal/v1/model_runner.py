...
class MetalModelRunner:
    ...
    def __init__(
        ...
        # AgentCache centroid injection (synthetic system-prefix warm-start).
        # Loaded once; no-op unless VLLM_CENTROID_SCHEDULER=1 and centroid
        # .npy files exist. Per-layer rope handles are resolved lazily on the
        # first seed (the model is not built yet at __init__).
        from vllm_metal.centroid.integration_mlx import (
            try_load_metal_centroid_injector,
        )

        self._centroid_injector = try_load_metal_centroid_injector()
        self._centroid_layer_ropes: list | None = None
    ...
    def _start_paged_forward(
        ...
        # ---- AgentCache centroid injection ----
        # Seed the synthetic prefix K/V into the gap region the scheduler marked
        # already-computed, before the model forward reads the cache. No-op
        # unless centroid mode is enabled. prefill_reqs carry block_ids /
        # start_pos (= num_computed_tokens) / req_id.
        if self._centroid_injector is not None:
            from vllm_metal.centroid.integration_mlx import apply_metal_centroid
