...
macOS-port copy: recognizes the HuggingFace/MPS mode names ("cold", "inject") emitted
by hf_eval.py, in addition to the original vLLM mode names. Schema is otherwise
identical, so the same tables apply.

Usage:
    python analyze_results.py <path_to_jsonl>
    python analyze_results.py results/Llama-3.2-1B_N64_2000.jsonl
"""
...
MODE_LABELS = {
    # macOS port (hf_eval.py)
    "cold":   "Cold (no prefix)",
    "inject": "Synthetic prefix (inject)",
    # original vLLM modes (kept so this analyzer reads both)
    "cold_no_synthetic": "Cold (no cache)",
    "warm_apc":          "Warm APC",
    "synthetic_compression": "Synthetic Compression",
}

# Baseline / candidate mode names for the per-example TTFT comparison, tried in order.
_BASELINE_MODES = ["cold", "cold_no_synthetic"]
_CANDIDATE_MODES = ["inject", "synthetic_compression"]
...
def analyze(path: str):
    ...
    baseline_mode = next((m for m in _BASELINE_MODES if m in ttft), None)
    if baseline_mode:
    ...
    candidate_mode = next((m for m in _CANDIDATE_MODES if m in data), None)
    if baseline_mode and candidate_mode and len(modes) >= 2:
        print_section("Per-example TTFT: Cold vs Synthetic prefix")
        cold_by_id = {r["id"]: r["ttft_mean_s"] for r in data[baseline_mode]}
        comp_by_id = {r["id"]: r["ttft_mean_s"] for r in data[candidate_mode]}
