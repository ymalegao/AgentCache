    ...
    def add_requests(self, scheduler_output: SchedulerOutput) -> None:
        for new_req_data in scheduler_output.scheduled_new_reqs:
            ...
            if not self.is_pooling_model and not (
                ...
                num_computed = centroid_override_num_computed(
                    num_computed, self._centroid_injector
                )
    ...
    def execute_model(
        ...
        if not dummy_run:
            ...
            if not self.is_pooling_model:
                bt_list = list(block_tables)
                bt_list[0] = apply_centroid_block_table(
                    self,
                    bt_list[0],
                    input_batch.num_reqs,
                    input_batch,
                )
                block_tables = tuple(bt_list)
